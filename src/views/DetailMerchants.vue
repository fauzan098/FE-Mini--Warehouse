<template>
  <Layout
    ...
    breadcrumb-text="Merchant Details"
    ...
    <main class="flex flex-col gap-6 flex-1">
      ...
      <section id="Products" class="flex flex-col gap-6 flex-1 rounded-3xl p-[18px] px-0 bg-white">
        <div id="Header" class="flex items-center justify-between px-[18px]">
          ...
          <router-link
            :to="`/assign-merchants-products/${merchant.id}`"
            ...
            <template v-for="product in products" :key="product.id">
              <div class="card flex items-center justify-between gap-6">
                <div class="flex items-center gap-3 w-[260px] shrink-0">
                  ...
                  <div class="flex flex-col gap-2 flex-1">
                    <p class="font-semibold text-xl w-[162px] truncate">{{ product.name }}</p>
                ...
                <div class="flex items-center gap-4">
                  ...
                  <router-link
                    :to="{
                      path: `/update-stock-merchants/${merchant.id}/stock/${product.product_id}`,
                      query: {
                        ...
                        keeper_name: merchant.keeper
                          ? merchant.keeper.name
                          : 'Keeper tidak ditemukan',
                      },
                    }"
      ...
      >
        <div class="flex items-center justify-between">
          <div class="flex flex-col gap-2">
            <p class="flex items-center gap-[6px] font-semibold text-lg">
              <img
                :src="getCategoryIcon(selectedProduct.product_category)"
                class="size-6 flex shrink-0"
                alt="icon"
              />
        ...
        <div>
          ...
          <p class="font-semibold leading-[160%]">
            {{ selectedProduct.product_about || 'No description available.' }}
          </p>
        </div>
        <hr class="border-monday-border" />
        <div>
          ...
          <div class="flex items-center gap-2 mt-2">
            ...
            <div class="flex flex-col">
              ...
              <p class="font-medium text-xs text-monday-gray">
                {{ selectedProduct.warehouse_phone }}
              </p>
            </div>
          </div>
        </div>
  ...
  methods: {
    ...
    async loadMerchantProducts(page = 1) {
      ...
      try {
        const query =
          page > 1
            ? `?page=${page}&merchant_id=${this.merchant.id}`
